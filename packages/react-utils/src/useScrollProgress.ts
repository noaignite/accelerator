...
export function useScrollProgress(
  ...
  const { container, when = true } = options
  ...
  /**
   * Observe scroll container resize when it's a custom element.
   */
  useResizeObserver(containerRef, scheduleUpdate, {
    when: when && Boolean(container),
  })

  /**
   * Observe scroll target resize.
   */
  useResizeObserver(ref, scheduleUpdate, { when })

  /**
   * Attach scroll listener (and window resize for viewport) when active (`when` is true).
   */
  useEffect(() => {
    if (!when) return
