...
import { assert, createCacheAside } from '@noaignite/utils'
  ...
  return {
    ...
    fetchProducts: async (sessionContext: SessionContext, ids: string[]) => {
      ...
      if (!isSessionContextConfigured) {
        ...
        return fetchProductsCentra<TProduct>(centraCheckoutCredentials, sessionContext, ids).then(
          (products) => products?.map(formatProduct) ?? [],
        )
      }

      return createCacheAside({
        cache: {
          get: (idsInner: string[]) => cache.get(sessionContext, idsInner),
          set: (products: TProduct[]) => cache.set(sessionContext, products),
        },
        fallback: async (foundEntries: TProduct[], idsInner: string[]) => {
          // Get `products` that are not found in cache
          const idsNotFoundInCache = idsInner.filter(
            (id) => !foundEntries.some((entry) => entry.product === id),
          )

          if (idsNotFoundInCache.length === 0) {
            // If there's no products to fetch, then we don't need to make a request.
            return []
          }

          return fetchProductsCentra(
            centraCheckoutCredentials,
            sessionContext,
            idsNotFoundInCache,
          ).then((products) => products?.map(formatProduct) ?? [])
        },
      })(ids).then((productsInner) => {
        return productsInner.sort((a, b) => {
          const aIdx = ids.findIndex((id) => id === a.product)
          const bIdx = ids.findIndex((id) => id === b.product)
