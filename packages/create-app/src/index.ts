...
const TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    display: 'Prettier Commit Hook',
    customCommands: [
      'npm install -w -D husky lint-staged',
      'npm exec husky init',
      ...
      [
        {
          type: 'text',
          name: 'rootDir',
          message: 'Where is the turborepo root directory located?',
          initial: '.',
        ...
        },
        {
          type: (_, { rootDir }: { rootDir: string }) => {
            ...
            if (!fs.existsSync(path.join(rootPath, 'turbo.json'))) {
              throw new Error(
                [
                  `${red('✖')} No ${bold('turbo.json')} found at ${bold(rootPath)}`,
        ...
        },
        {
          type: 'multiselect',
          name: 'templates',
          message: 'Select what templates to install:',
